...
**【2026年 AI/SGE時代のH2設計ルール】**
- **Standalone Summaries**: 各H2は、それ単体で「検索意図への回答」が完結する独立した要約として機能させること（エモいポエム見出しは禁止）。
- **クエリの自然な配置**: 最も重要なH2（特に最初のH2やFAQ）には、ターゲットKW（生の検索語彙）を自然な形で必ず含めること。ただし、すべてのH2に機械的にKWを詰め込む過剰最適化（Keyword Stuffing）は避けること。
