...
- 生成候補の一時置き場: `/private/tmp/seiribu-image-work/kaitori-kaishu-order`
- 採用画像の公開用置き場: `seiribu-editorial/assets/images/kaitori-kaishu-order`
...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | アイキャッチ素材 | 画像生成素材 + 背景透過（ローカル合成で仕上げ） | 1200 x 675 | 16:9 | `kaitori-kaishu-order-eyecatch.png` | アイキャッチ合成用素材 |
| 2 | 記事内イメージ | 画像生成素材 | 1200 x 675 | 16:9 | `kaitori-kaishu-order-inline-01.png` | 本文中のCMSブリーフ位置 |

## 保留した画像

| 用途 | ファイル名 | 保留理由 |
| --- | --- | --- |
| 記事内イメージ | `kaitori-kaishu-order-inline-male-memory.png` | lightモードでは本文画像を1件までに絞ります。必要なら --mode standard で出力します。 |
...
- ファイル名: `kaitori-kaishu-order-inline-01.png`
...
- imagegenの生成候補は `/private/tmp/seiribu-image-work/kaitori-kaishu-order` に置き、採用画像だけ `seiribu-editorial/assets/images/kaitori-kaishu-order` に移す。
- 公開用フォルダに残った候補や旧版は `seiribu-editorial/scripts/clean_image_assets.py --article kaitori-kaishu-order --dry-run` で確認してから退避する。
